...
**每次開跑前，先確認「存到哪裡」**——預設 `~/Downloads`（即 `/Users/<你>/Downloads`），如要改路徑現在講。確定後在該根路徑下建立第 1 節的資料夾骨架。
...
3. **顯示提示語並停下等手動**：
   > 「切回 Figma 儲存『印刷-票口海報_67x108mm@4x』至 `3-票口海報/印刷/Links/`，待我手動存。」
4. **偵測**：確認 `Links/印刷-票口海報_67x108mm@4x.png` 已存在 → 才執行下一步。
...
7. [ ] 票口海報：社群版 1x JPG **自動**輸出 → 顯示提示語請設計師切回 Figma 手動存 `印刷-票口海報_67x108mm@4x.png` 至 `印刷/Links/` → **偵測到該檔已存** → 跑 `票口海報印刷封裝.command`（複製 .ai＋重嵌主圖＋產 K100；Illustrator 啟動時提示「點一下 Illustrator 並切回這裡」）。
8. [ ] 全部對照本表清點數量。
