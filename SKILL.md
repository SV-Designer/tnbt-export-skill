...
2. **確認輸出根路徑**（預設 `outputBase`，通常 `~/Downloads`，要改現在講）。
...
2. **顯示提示語**：「切回 Figma 儲存『印刷-票口海報_67x108mm@4x』至 `3-票口海報/印刷/Links/`，待我手動存。」
3. **偵測** `Links/印刷-票口海報_67x108mm@4x.png` 已存在 → 才往下。
4. 跑 `scripts/票口海報印刷封裝.command "<印刷資料夾>"`：複製範本 .ai → Illustrator 重嵌主圖（啟動時提示「點一下 Illustrator 並切回」）→ 產 K100 黑底版。

**Step 7 — 清點**：對照 `docs/SOP.md` §1 數量表清查。
